...
if args.prepare:
  if args.llvm_url is None or args.boost_url is None:
    print(f"need url to download llvm and boost, see --help to get the right url")
    exit(0)
  DO("rm -rf ./boost*")
  DO("rm -rf ./*LLVM*")
...
if args.build:
  DO("docker container ls -a | grep 'dev' | awk '{print $1}' | xargs -r docker container stop | xargs -r docker rm") #如果有正在运行的docker，结束掉
  DO('''docker image ls | grep 'dev' | awk '{print $1":"$2}' | xargs -r docker image rm''') #删除已有的同名镜像
  DO("docker build -t dev:v2 -f Dockerfile .") #创建新的镜像
if args.run:
  DO("docker run -d --mount type=bind,source=$HOME/Desktop/src,target=/root/project -p 1024:22 dev:v2")
